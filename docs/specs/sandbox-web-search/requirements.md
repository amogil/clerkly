...
2.8. КОГДА helper вызывается с несколькими запросами (OpenAI/Google `queries`), ТО запросы ДОЛЖНЫ выполняться последовательно. ЕСЛИ один из запросов завершился ошибкой, ТО выполнение ДОЛЖНО прекратиться немедленно (fail-fast): оставшиеся запросы не выполняются, частичные результаты не возвращаются.
