...
#### 1) Спеки и критерии
- [x] Уточнить acceptance-критерий в `agents`/`llm-integration`: иконка в reasoning-trigger анимирована только во время активного reasoning; после завершения reasoning и автосворачивания остаётся статичной.
- [x] Добавить/обновить ссылки на функциональный тест в секциях "Функциональные Тесты".
- [x] Обновить таблицы покрытия требований в `design.md`.

#### 2) Репродукция и фиксация текущего поведения
- [x] Запустить точечный functional-сценарий reasoning-trigger и зафиксировать переходы: streaming → finish → collapse.
- [x] Подтвердить единый источник truth для состояния "reasoning завершён" в renderer.

#### 3) Тесты сначала (red → green)
- [x] Добавить unit-тест на переход состояния trigger: после завершения reasoning и collapsed-состояния `Logo` получает `animated=false`.
- [x] Добавить functional-тест: после появления финального `action.content` и автосворачивания trigger в DOM отсутствует `svg.logo-animated`.
- [x] Проверить отсутствие регрессий toggle-поведения (manual collapse/expand).

#### 4) Изменение реализации (точечно)
- [x] Обновить вычисление флага анимации в `AgentReasoningTrigger` (и при необходимости связанный источник флага в `AgentChat`) так, чтобы анимация зависела только от активной reasoning-фазы.
- [x] Убедиться, что текст, chevron и поведение раскрытия/сворачивания не меняются.

#### 5) Валидация
- [x] Запустить только затронутые unit-тесты.
- [x] Запустить только затронутые functional-тесты (с предупреждением о показе окон).
