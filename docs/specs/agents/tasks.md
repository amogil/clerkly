...
**Текущий статус:** Фаза 6 — Валидация и сдача
...
### В Работе
- 🔄 Подготовка итоговой сводки по доработке после целевого functional-прогона.
...
- [ ] При необходимости уточнить связанное поведение в `docs/specs/llm-integration/design.md`/`requirements.md` для ручной отмены через кнопку.
...
- [ ] Отдельно спросить пользователя о запуске функциональных тестов (они показывают окна).
