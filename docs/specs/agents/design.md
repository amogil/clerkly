...
- Shortcut hint `Press Enter to send, Shift+Enter for new line` отображается внутри `"PromptInputFooter"` в составе `"PromptInputTools"`, выравнивается по той же левой колонке, что и текст `"PromptInputTextarea"`, использует менее акцентную уменьшенную типографику, выравнивается по нижнему краю footer-строки и получает ещё более выраженный additional vertical offset вниз, чтобы сидеть максимально близко к нижней границе `"PromptInput"`; сама рамка `"PromptInput"` отделена от сообщений увеличенным верхним inset (`mt-4`).
...
```bash
npm run ai-elements:update-all
```
