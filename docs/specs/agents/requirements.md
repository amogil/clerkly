...
7.4.2. `tool_call` с `toolName = "final_answer"` ДОЛЖЕН содержать:
  - краткий completion-текст (не дублирующий полный пользовательский ответ из `kind: llm`),
  - опциональное краткое резюме в виде `summary_points` о выполненных действиях.

7.4.2.1. Контракт и лимиты аргументов `final_answer` ДОЛЖНЫ соответствовать `llm-integration.9.5.*`.

7.4.3. КОГДА `tool_call` с `toolName = "final_answer"` нарушает контракт `llm-integration.9.5.*`, ТО такой `tool_call` НЕ ДОЛЖЕН фиксироваться как успешный `completed`; система ДОЛЖНА запускать retry и при исчерпании retry создавать `kind:error`.
