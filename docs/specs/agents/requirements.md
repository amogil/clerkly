...
7.4.6.11. Для входного кода `tool_call(code_exec)` родительская рамка встроенного markdown code block (контейнер с language-header) НЕ ДОЛЖНА рендериться; при этом внутренняя рамка области кода ДОЛЖНА сохраняться.
