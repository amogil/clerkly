...
11.1. КОГДА активное `kind: llm` сообщение имеет `done = false`, ТО ДОЛЖЕН отображаться индикатор активности (`in-progress`)
...
11.4. Индикатор ДОЛЖЕН исчезать, когда активное `kind: llm` сообщение переходит в `done = true` ИЛИ скрывается через `hidden: true`
