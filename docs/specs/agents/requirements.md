...
7.2.2. Для сценария завершения задачи моделью ДОЛЖНО сохраняться сообщение `kind: tool_call` c `toolName = "final_answer"` и `done = true`.
...
7.4.1. `tool_call` с `toolName = "final_answer"` ДОЛЖЕН отображаться как обычное сообщение модели (визуально как `kind: llm`) с дополнительным визуальным индикатором завершения (`Completed` badge).

7.4.2. `tool_call` с `toolName = "final_answer"` ДОЛЖЕН содержать:
  - текст итогового ответа модели,
  - опциональное краткое резюме в виде `summary_points` (от 0 до 10 пунктов) о выполненных действиях.

7.4.2.1. КАЖДЫЙ пункт `summary_points` ДОЛЖЕН иметь длину не более 200 символов.

7.4.2.1.1. Ограничения `summary_points` (до 10 пунктов, до 200 символов на пункт) относятся к целевому контракту генерации модели; при нарушении этих ограничений UI ДОЛЖЕН отображать полученные данные как есть (без обрезки и без отбрасывания пунктов).

7.4.2.2. ОТСУТСТВИЕ `summary_points` (или пустой массив) при наличии непустого текста ответа ДОЛЖНО считаться успешным завершением (`completed`).

7.4.3. КОГДА `tool_call` с `toolName = "final_answer"` приходит без текста или с пустым текстом, ТО UI ДОЛЖЕН показывать стандартный fallback-текст: `Модель закончила работу` (без показа error bubble).

7.4.4. Для `tool_call(final_answer)` UI ДОЛЖЕН иметь отдельные тестовые идентификаторы:
  - `data-testid="message-completed-badge"` для бейджа `Completed`,
  - `data-testid="message-completed-summary"` для контейнера списка `summary_points`.
