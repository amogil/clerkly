...
7.4.6.5.4. Секция `stdout` в `tool_call(code_exec)` ДОЛЖНА отображать стандартный заголовок code block `Output` внутри соответствующего блока; секция `stderr` ДОЛЖНА отображать стандартный заголовок code block `Error`.
