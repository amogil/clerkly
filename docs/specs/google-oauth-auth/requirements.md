...
### Требование 17: Инкрементальная Re-Авторизация (Incremental Re-Authorization)

**ID:** google-oauth-auth.17

**User Story:** Как система, я хочу запрашивать дополнительные Google scopes у пользователя без потери текущей сессии, чтобы агент мог получить доступ к новым API по запросу.

#### Критерии Приемки

17.1. КОГДА система определяет необходимость дополнительных Google scopes, ТО "OAuth Client" ДОЛЖЕН построить authorization URL с объединением текущих и запрошенных scopes И параметром `include_granted_scopes=true`

17.2. КОГДА инкрементальная re-авторизация завершается успешно, ТО "OAuth Client" ДОЛЖЕН обновить access token и refresh token в хранилище И сохранить полный список granted scopes

17.3. КОГДА инкрементальная re-авторизация завершается ошибкой (пользователь отменяет, сетевая ошибка и т.д.), ТО система НЕ ДОЛЖНА терять существующие токены и scopes

17.4. Инкрементальная re-авторизация ДОЛЖНА переиспользовать существующий PKCE flow и deep link handler

17.5. КОГДА инкрементальная re-авторизация активна, ТО система ДОЛЖНА различать её от первичной авторизации, чтобы при ошибке не очищать существующую сессию
