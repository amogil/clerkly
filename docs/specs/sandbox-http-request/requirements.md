...
1.3.1. В рамках одного выполнения `code_exec` sandbox-код МОЖЕТ вызывать helper `http_request` несколько раз

1.3.2. ГДЕ несколько вызовов `http_request` независимы друг от друга, sandbox-код МОЖЕТ комбинировать их конкурентно через конструкции вида `Promise.all(...)`, при условии соблюдения тех же лимитов и policy
...
2.5.2. ЕСЛИ поле `headers` содержит request-control или hop-by-hop заголовки, включая `host`, `content-length`, `connection`, `proxy-connection`, `transfer-encoding`, `upgrade`, `keep-alive`, `te`, `trailer` или `expect`, ТО helper ДОЛЖЕН отклонять вызов как невалидный
...
3.2.1. КОГДА hostname резолвится в один или несколько допустимых публичных адресов, ТО система ДОЛЖНА привязывать фактическое исходящее соединение только к уже проверенным адресам, а НЕ выполнять независимый повторный DNS-resolve в момент сетевого подключения

3.2.2. ЕСЛИ hostname резолвится в несколько допустимых публичных адресов, ТО система ДОЛЖНА пытаться использовать эти проверенные адреса в порядке, возвращённом resolver-ом, пока не будет установлено первое успешное соединение или пока список не исчерпается
