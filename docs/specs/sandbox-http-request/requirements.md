...
3.2.1. КОГДА hostname резолвится в допустимый публичный адрес, ТО система ДОЛЖНА привязывать фактическое исходящее соединение к этому уже проверенному адресу, а НЕ выполнять независимый повторный DNS-resolve в момент сетевого подключения
