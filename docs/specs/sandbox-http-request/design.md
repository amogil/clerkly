...
9. Handler определяет формат тела ответа:
...
10. Handler формирует структурированный результат.
11. Handler усекает итоговый `body` по применённому лимиту и выставляет `truncated`.
12. Handler возвращает structured result обратно в sandbox-код.
...
- `tests/unit/code_exec/SandboxHttpRequestHandler.test.ts` - проверяет request validation, timeout, redirects, fetch-compatible redirect rewriting, truncation, error mapping
