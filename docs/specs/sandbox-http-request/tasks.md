...
- Параметр `max_response_bytes` должен быть опциональным; если он передан, helper использует его как явный лимит ответа в байтах, а если не передан — применяет внутренний safety cap.
...
- ✅ Уточнено, что `max_response_bytes` опционален, а при его отсутствии helper применяет внутренний safety cap.
