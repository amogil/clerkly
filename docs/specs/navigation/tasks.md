...
**Выполнено:** Все задачи, кроме одного функционального теста

**Не выполнено:**
1. Функциональный тест 6.8 - Показ ошибки при неудаче авторизации

**Примечание:** Оставшаяся задача относится к функциональному тесту. Функциональность покрыта другими тестами (auth-flow.spec.ts).
