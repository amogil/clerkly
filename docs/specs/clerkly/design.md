...
Clerkly - это Electron-приложение для Mac OS X, предназначенное для менеджеров. Реализация включает локальное хранение данных, нативный интерфейс, AI-чат с потоковой генерацией и tool calling, а также комплексное тестовое покрытие.
