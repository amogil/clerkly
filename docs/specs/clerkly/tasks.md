...
- [x] 3. Реализация централизованного Logger класса
  ...
  - [x] 3.5 Рефакторинг кода для использования параметризованного Logger
...
В рамках рефакторинга системы хранения данных (см. `docs/specs/database-refactoring/tasks.md`), необходимо обновить компоненты Clerkly для использования новой архитектуры:
...
- **Query API:** DatabaseManager предоставляет методы `runUserQuery`, `getUserRow`, `getUserRows` для запросов с автоматической подстановкой user_id, и `runQuery`, `getRow`, `getRows` для глобальных запросов (см. `docs/specs/database-manager-refactoring/tasks.md`)
