...
- provider-layer не вводит скрытый лимит числа шагов tool-loop; для SDK-managed continuation провайдеры используют semantic stop condition по `final_answer`, поэтому остановка turn определяется только доменными исходами orchestration (`final_answer`, ошибка, abort/cancel).
...
- score guard: rename разрешён только при `rename_need_score >= 80`;
...
11) `final_answer` отображается последним артефактом успешной попытки независимо от порядка прихода в provider-stream.
...
- при завершении через `final_answer` модель не публикует перед вызовом инструмента отдельный обычный markdown/text summary, буллеты или checklist с теми же solved-task пунктами;
