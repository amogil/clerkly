...
- ЕСЛИ turn eligible, instruction вставляется как `role: system` через `injectSystemMessage(...)` и содержит:
  ...
  - запрет на использование парных знаков препинания (кавычек, скобок, backticks, угловых скобок) в генерируемом title (defense-in-depth: `llm-integration.16.8.4`).
...
- Удаление непарных парных знаков препинания (paired punctuation): после удаления краевой пунктуации выполняется проверка баланса для каждого типа парных символов:
  - ASCII `"`: если количество нечётное — все удаляются.
  - Типографские двойные кавычки `\u201C`/`\u201D`: если количество left не равно количеству right — все удаляются.
  - Типографские одинарные кавычки `\u2018`/`\u2019`: если количество left не равно количеству right — все удаляются.
  - Круглые скобки `(`/`)`: если количество открывающих не равно количеству закрывающих — все удаляются.
  - Квадратные скобки `[`/`]`: если количество открывающих не равно количеству закрывающих — все удаляются.
  - Фигурные скобки `{`/`}`: если количество открывающих не равно количеству закрывающих — все удаляются.
  - Обратные кавычки `` ` ``: если количество нечётное — все удаляются.
  - Угловые скобки `<`/`>`: если количество открывающих не равно количеству закрывающих — все удаляются.
  - ASCII одинарные кавычки / апострофы (`'`, U+0027) не затрагиваются.
  - После удаления результат повторно trimится.
...
### Финализация running tool calls при retry (llm-integration.11.6.2)

При retry через `handleAttemptFailure` running tool_call записи (`state.runningToolCalls`) финализируются ДО скрытия сообщений попытки:

1. `finalizeRunningToolCallsForAttempt(state, agentId)` итерирует `state.runningToolCalls` (Map).
2. Для каждой записи формирует terminal output:
   - `code_exec`: `{ status: 'error', stdout: '', stderr: '', stdout_truncated: false, stderr_truncated: false, error: { code: 'internal_error', message: 'Retried due to model error.' } }`
   - прочие: `{ status: 'error', content: 'Retried due to model error.' }`
3. Вызывает `messageManager.update(messageId, agentId, payload, true)` для каждой записи.
4. Очищает `state.runningToolCalls`.
5. Вызывается перед циклом `setHidden` в `handleAttemptFailure`.

### Startup reconciliation (llm-integration.11.6.3)

При старте приложения система финализирует все stale `tool_call` записи:

1. `MessagesRepository.listStaleToolCalls()` выполняет SQL:
   `SELECT * FROM messages WHERE done=false AND kind='tool_call' AND agentId IN (SELECT agentId FROM agents WHERE userId=<currentUser>)`
   Возвращает все stale tool_call rows (включая `hidden=true`).

2. `MessageManager.finalizeAllStaleToolCallsOnStartup()`:
   - Вызывает `MessagesRepository.listStaleToolCalls()`.
   - Для каждой записи: parse payload, set terminal output по toolName (status: `cancelled`), update через `MessageManager.update()`.
   - Emit'ит `MessageUpdatedEvent` для каждой обновлённой записи через стандартный `MessageManager.update()`. На момент вызова renderer ещё не подключён и `BrowserWindow` не создан, поэтому события доставляются только в main-процесс (в `MainEventBus` local subscribers); IPC broadcast в renderer — no-op. Это обеспечивает согласованность: все мутации сообщений проходят через единый event-emitting путь, что позволяет будущим pre-renderer подписчикам (аналитика, аудит, синхронизация) видеть операции startup reconciliation.
   - Logging: `this.logger.info(...)` с количеством обработанных записей.

3. Вызов в `src/main/index.ts`: после `userManager.initialize()`, до `appCoordinator.start()`.
   Условие: `userManager.getCurrentUserId()` не null.

### Startup reconciliation for stale kind:llm (llm-integration.11.6.4)

При старте приложения система скрывает все stale `kind:llm` сообщения:

1. `MessagesRepository.listStaleLlmMessages()` выполняет SQL:
   `SELECT * FROM messages INNER JOIN agents ON messages.agentId = agents.agentId WHERE agents.userId = ? AND messages.kind = 'llm' AND messages.done = false AND messages.hidden = false ORDER BY messages.id ASC`
   Возвращает все stale `kind:llm` rows с `done=false` и `hidden=false`.
   В отличие от `listStaleToolCalls()`, которая включает `hidden=true` строки (tool calls нужно перевести в terminal-состояние), этот метод фильтрует только `hidden=false`, так как `hidden=true` llm-сообщения уже в корректном состоянии.

2. `MessageManager.hideAllStaleLlmOnStartup()`:
   - Вызывает `MessagesRepository.listStaleLlmMessages()`.
   - Для каждой записи: вызывает `MessageManager.setHidden(row.id, row.agentId)` для установки `hidden=true`.
   - Флаг `done` остаётся `false` (семантика `hideAndMarkIncomplete`).
   - Emit'ит `MessageUpdatedEvent` для каждой скрытой записи через стандартный `MessageManager.setHidden()`. Аналогично `finalizeAllStaleToolCallsOnStartup()`, на момент вызова renderer ещё не подключён — события доставляются только в main-процесс local subscribers.
   - Logging: `this.logger.info(...)` с количеством обработанных записей.

3. Вызов в `src/main/index.ts`: после `messageManager.finalizeAllStaleToolCallsOnStartup()`, в том же `if (userManager.getCurrentUserId())` guard.
...
- Для non-timeout ошибок: pipeline-level retry ограничен одним повтором на один запуск `MainPipeline.run()`.
...
- Для timeout-ошибок (`normalizeLLMError(error).type === 'timeout'`) до появления первого meaningful chunk: pipeline-level retry до 3 consecutive повторов (4 попытки суммарно, включая начальную).
- Счётчик consecutive timeout-повторов (`consecutiveTimeouts` в `AttemptCycleState`) сбрасывается при успешной попытке — не является сквозным через весь run.
- Timeout retry НЕ выполняется при user-abort (`signal.aborted`).
- После исчерпания 3 consecutive timeout retry создаётся стандартное `kind:error` с `type=timeout`.
- Константа `MAX_TIMEOUT_RETRIES = 3`.
...
- `tests/unit/agents/MessageManager.test.ts` — `finalizeAllStaleToolCallsOnStartup`: финализация visible stale tool_call, финализация hidden stale tool_call, пропуск terminal (done=true), no-op без stale записей, code_exec cancelled output, non-code_exec cancelled output, emit'ит MessageUpdatedEvent для каждой финализированной записи, malformed payloadJson fallback, использует MessageManager.update (emits events), обработка множественных stale rows из разных агентов
- `tests/unit/agents/MessageManager.test.ts` — `hideAllStaleLlmOnStartup`: скрытие visible stale llm (hidden=false, done=false), no-op без stale llm, обработка множественных stale rows из разных агентов, emit'ит MessageUpdatedEvent для каждой скрытой записи, использует MessageManager.setHidden (emits events), логирование количества скрытых строк
- `tests/unit/db/repositories/MessagesRepository.test.ts` — `listStaleLlmMessages`: возвращает kind:llm с done=false и hidden=false, исключает done=true, исключает hidden=true, исключает другие kinds (user, error, tool_call), фильтрация по текущему пользователю через agents join
...
- `tests/unit/agents/AgentTitlePromptContract.test.ts` — проверка наличия запрета парных знаков препинания в промпте auto-title (defense-in-depth: `llm-integration.16.8.4`)
...
| llm-integration.12.2.3 | ✓ | - |
| llm-integration.12.2.4 | ✓ | - |
...
| llm-integration.16.8.3 | ✓ | - |
| llm-integration.16.8.4 | ✓ | - |
