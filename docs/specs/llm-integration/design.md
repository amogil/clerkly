...
- Удаление непарных двойных кавычек: после удаления краевой пунктуации подсчитываются вхождения каждого типа двойных кавычек (ASCII `"`, типографские `\u201C`/`\u201D`). Если количество ASCII `"` нечётное — все ASCII `"` удаляются. Если количество типографских left `\u201C` не равно количеству right `\u201D` — все типографские кавычки удаляются. Результат повторно trimится. Одинарные кавычки и апострофы не затрагиваются.
...
- `tests/unit/agents/AgentTitleNormalization.test.ts` — нормализация/валидация title (single-line, trim, punctuation, max 200, удаление непарных двойных кавычек)
