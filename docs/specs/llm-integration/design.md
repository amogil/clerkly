...
- Извлечение auto-title для агента выполняется в том же model-turn из markdown-ответа (`<!-- clerkly:title: ... -->`) без отдельного LLM-запроса.
...
- ЕСЛИ текущий title равен `New Agent` и triggering user-message не проходит meaningful-check (>=3 буквенно-цифровых символа), per-turn auto-title instruction НЕ добавляется.
- ЕСЛИ turn eligible, instruction вставляется как `role: system` через `injectSystemMessage(...)` и содержит:
  - контракт формата `<!-- clerkly:title: ... -->`;
  - контекст `Current chat title: "<текущее имя>"`.

**Контракт метаданных в ответе модели:**
- Комментарий формата `<!-- clerkly:title: ... -->`.
- Комментарий может находиться в любом месте markdown-ответа.
...
2. При обнаружении префикса `<!-- clerkly:title:` parser переходит в `capture`.
3. В `capture` parser накапливает payload до:
   - закрывающего `-->`, или
   - достижения `TITLE_COMMENT_PAYLOAD_MAX_LENGTH = 200`.
4. Если `-->` найден до лимита:
   - фиксируется candidate title,
   - stream ответа НЕ модифицируется (комментарий остаётся в markdown-потоке),
   - parser больше не извлекает title в текущем turn.
5. Если лимит 200 достигнут без `-->`:
...
- Для auto-title используется единый лимит длины 200 символов без отдельного ограничения по количеству слов.
...
- initial-rename guard: ПОКА текущий заголовок равен `New Agent`, auto-rename разрешён только при meaningful user-message (минимум 3 буквенно-цифровых символа).
...
10) `final_answer` отображается последним артефактом успешной попытки независимо от порядка прихода в provider-stream.
...
- `tests/unit/agents/MainPipeline.test.ts` — parser комментария `<!-- clerkly:title: ... -->` (search/capture, split chunks, незакрытый comment, лимит 200)
...
- `tests/unit/agents/AgentTitleAntiFlap.test.ts` — guards anti-flapping (exact match, Jaccard threshold, cooldown)
