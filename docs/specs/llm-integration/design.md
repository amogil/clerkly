...
- provider-layer не вводит скрытый лимит числа шагов tool-loop; остановка turn определяется только доменными исходами orchestration (`final_answer`, ошибка, abort/cancel).
