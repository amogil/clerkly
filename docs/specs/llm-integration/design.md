...
11) Для `final_answer.summary_points` pipeline не выполняет отдельную selective runtime-дедупликацию narrative markdown/text сегментов; предотвращение смысловых дублей обеспечивается системной инструкцией модели.
...
- при завершении через `final_answer` модель может публиковать обычный markdown/text ответ, но этот текст не должен семантически дублировать `final_answer.summary_points`;
- это правило задаётся инструкцией в prompt/tool description и не требует отдельного selective пост-обрезания narrative markdown/text на runtime-уровне;
