...
2. `MessageManager.finalizeAllStaleToolCallsOnStartup()`:
   ...
   - Для каждой записи: parse payload, set terminal output по toolName (status: `cancelled`), update через `MessagesRepository.update()`.
   - Не emit'ит `MessageUpdatedEvent` — renderer ещё не подключён.
...
2. `MessageManager.hideAllStaleLlmOnStartup()`:
   ...
   - Для каждой записи: вызывает `MessagesRepository.setHidden(row.id, row.agentId)` для установки `hidden=true`.
...
- `tests/unit/agents/MessageManager.test.ts` — `finalizeAllStaleToolCallsOnStartup`: финализация visible stale tool_call, финализация hidden stale tool_call, пропуск terminal (done=true), no-op без stale записей, code_exec cancelled output, non-code_exec cancelled output, не emit'ит MessageUpdatedEvent, malformed payloadJson fallback, bypass MessageManager.update (repo direct), обработка множественных stale rows из разных агентов
- `tests/unit/agents/MessageManager.test.ts` — `hideAllStaleLlmOnStartup`: скрытие visible stale llm (hidden=false, done=false), no-op без stale llm, обработка множественных stale rows из разных агентов, не emit'ит MessageUpdatedEvent, использует MessagesRepository.setHidden напрямую, логирование количества скрытых строк
