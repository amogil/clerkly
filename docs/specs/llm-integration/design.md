...
- score guard:
  - при текущем заголовке `New Agent` rename разрешён только при `rename_need_score > 50`;
  - при текущем заголовке, отличном от `New Agent`, rename разрешён только при `rename_need_score >= 80`;
