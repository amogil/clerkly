...
5) Эмитит `message.created`/`message.updated` как обычно (отдельных событий для изображений нет).
6) Запускает фоновую загрузку изображений внутри `MainPipeline` (без отдельного класса).
...
- URL недоступен / ошибка сети → fallback + лог.
- content‑type не `image/*` → отказ с fallback.
- размер > лимита → отказ с fallback.
- повторные плейсхолдеры одного `id` → использовать кэш.
...
- Ошибка: неизвестный id → fallback.
