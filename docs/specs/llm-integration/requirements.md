...
9.5.1. `tool_call` с `toolName = "final_answer"` ДОЛЖЕН содержать:
  - краткий текст завершения задачи (не дублирующий полный основной ответ из `kind: llm`),
  - опциональное краткое резюме в `summary_points`.

9.5.2. Поле `text` в `final_answer` ДОЛЖНО быть непустой строкой длиной не более 280 символов.

9.5.3. `summary_points` ДОЛЖЕН содержать от 0 до 10 пунктов.

9.5.4. КАЖДЫЙ пункт `summary_points` ДОЛЖЕН иметь длину не более 200 символов.

9.5.5. ЕСЛИ `final_answer` нарушает ограничения по `text`/`summary_points`, ТО система ДОЛЖНА считать такой `final_answer` невалидным и запустить повтор запроса (retry) по правилам `llm-integration.12.*`.

9.5.6. ОТСУТСТВИЕ `summary_points` (или пустой массив) при наличии валидного непустого текста ДОЛЖНО считаться успешным `completed`.

9.6. В целевой модели невалидный `final_answer` НЕ ДОЛЖЕН отображаться в UI как завершённый ответ; он ДОЛЖЕН либо быть исправлен через retry, либо завершиться `kind:error` при исчерпании retry-лимита.
...
11.2.1. Для `final_answer` инструмент ДОЛЖЕН вызываться в strict-режиме через AI SDK; соблюдение контракта аргументов (`llm-integration.9.5.*`) ДОЛЖНО обеспечиваться схемой инструмента и встроенным retry/repair механизмом SDK.

11.3. Выполнение инструмента ДОЛЖНО сопровождаться сохранением сообщения `kind: tool_call` в `messages`; UI чата ДОЛЖЕН рендерить его как tool-call блок, кроме `toolName = "final_answer"` (для него применяется рендер completion summary с `Completed` badge).
...
11.6. UI ДОЛЖЕН отображать промежуточные `tool_call` сообщения как отдельные tool-call блоки; `tool_call(final_answer)` ДОЛЖЕН отображаться как блок завершения (`Completed` badge + completion summary), при этом полный пользовательский ответ модели остаётся в `kind: llm`. Параллельно пользователю показывается общий прогресс через статус `in-progress`, который определяется по последнему видимому сообщению (`kind: llm` или `kind: tool_call`) с `done = false`.
...
12.2.1. КОГДА получен невалидный `final_answer`, система ДОЛЖНА автоматически выполнять retry/repair через механизмы AI SDK.

12.2.2. Retry для невалидного `final_answer` ДОЛЖЕН быть ограничен конечным числом попыток (защита от бесконечного цикла).
