...
3.6. КАЖДЫЙ отдельный запрос к LLM API (от отправки до получения полного ответа модели) ДОЛЖЕН быть ограничен таймаутом 60 секунд. ЕСЛИ модель не завершила ответ за 60 секунд, ТО запрос прерывается с ошибкой таймаута

3.6.1. КОГДА в рамках одного pipeline-цикла выполняется несколько последовательных запросов к LLM API (например: запрос → tool call → повторный запрос), ТО таймаут 60 секунд применяется к **каждому запросу отдельно**. Время выполнения инструментов между запросами НЕ ДОЛЖНО учитываться в таймауте
