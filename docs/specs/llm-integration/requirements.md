...
3.6. Таймаут ожидания ответа от LLM ДОЛЖЕН составлять 60 секунд (1 минута) **на каждый отдельный model step**. ЕСЛИ модель не завершила текущий step за 60 секунд, ТО запрос прерывается с ошибкой таймаута
