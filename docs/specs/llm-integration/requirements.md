...
6.6. Миграции по полю `done` ДОЛЖНЫ быть обратносовместимыми и ДОЛЖНЫ сохранить корректную семантику завершённости для существующих сообщений.

6.6.1. Исторические сообщения `kind: llm`, у которых в payload уже присутствует финальный `data.action`, ДОЛЖНЫ быть помечены как завершённые (`done = true`) при миграции данных.
