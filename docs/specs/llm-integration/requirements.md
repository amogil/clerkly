...
9.5.1.1. Системная инструкция и описание инструмента `final_answer` ДОЛЖНЫ явно фиксировать:
  ...
  - `summary_points` соблюдает лимиты `llm-integration.9.5.2-9.5.3.1` и `llm-integration.9.5.3.4` и перечисляет решённые задачи.
...
9.5.3. КАЖДЫЙ пункт `summary_points` ДОЛЖЕН иметь длину не более 300 символов.
...
9.5.3.4. КАЖДЫЙ пункт `summary_points` ДОЛЖЕН заканчиваться полным словом или предложением; обрыв пункта на середине слова ЗАПРЕЩЁН. КОГДА пункт приближается к лимиту длины, модель ДОЛЖНА перефразировать его так, чтобы он укладывался в лимит без потери читабельности.
