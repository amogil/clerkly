...
9.5.1.1. Системная инструкция и описание инструмента `final_answer` ДОЛЖНЫ явно фиксировать, что:
  - `text` не дублирует полный основной ответ,
  - `text` и `summary_points` соблюдают лимиты контракта `llm-integration.9.5.*`.

9.5.2. Поле `text` в `final_answer` ДОЛЖНО быть непустой строкой длиной не более 300 символов.
...
9.5.5. ЕСЛИ `final_answer` нарушает ограничения по `text`/`summary_points`, ТО система ДОЛЖНА считать такой `final_answer` невалидным и запустить retry/repair по правилам `llm-integration.12.*`.
