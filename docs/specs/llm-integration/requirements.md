...
3.6.1. КОГДА в рамках одного pipeline-цикла выполняется несколько последовательных запросов к LLM API (например: запрос → tool call → повторный запрос), ТО таймаут применяется к **каждому запросу отдельно**. Время выполнения инструментов между запросами НЕ ДОЛЖНО учитываться в таймауте
...
12.2.3. КОГДА нормализованный тип ошибки = `timeout` И run не отменён пользователем И первый meaningful chunk (`reasoning`/`text`) ещё не получен, ТО `MainPipeline` ДОЛЖЕН выполнить до 3 consecutive повторных попыток (4 attempts total включая начальную). Счётчик consecutive timeout-повторов ДОЛЖЕН сбрасываться при успешной попытке. После исчерпания retry ДОЛЖЕН создаваться ровно один `kind:error` с `type=timeout`.

12.2.4. Существующее поведение retry для non-timeout recoverable ошибок (1 повтор до первого meaningful chunk) ДОЛЖНО оставаться неизменным.
