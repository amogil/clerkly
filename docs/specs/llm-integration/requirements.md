...
9.5.1.1. Системная инструкция и описание инструмента `final_answer` ДОЛЖНЫ явно фиксировать, что `summary_points` соблюдает лимиты `llm-integration.9.5.2-9.5.3`.
...
9.5.1.1.3. КОГДА вызывается `final_answer`, `summary_points` ДОЛЖЕН перечислять решённые задачи.

9.5.1.2. Значения по умолчанию для отсутствующих полей `final_answer` ДОЛЖНЫ быть:
  - `summary_points`: `[]`.

9.5.2. `summary_points` ДОЛЖЕН содержать от 0 до 10 пунктов.
...
9.5.5. ОТСУТСТВИЕ `summary_points` (или пустой массив) ДОЛЖНО считаться успешным `completed`; правила отображения определяются спецификацией `agents` (см. `agents.7.4`).
