...
3.6. Таймаут ожидания ответа от LLM ДОЛЖЕН составлять 300 секунд (5 минут) **на каждый отдельный model step**. ЕСЛИ модель не завершила текущий step за 300 секунд, ТО запрос прерывается с ошибкой таймаута

3.6.1. В multi-step tool loop (когда SDK автоматически выполняет tool и отправляет следующий запрос к модели), таймер ДОЛЖЕН сбрасываться после завершения каждого step (onStepFinish). Время выполнения инструментов НЕ ДОЛЖНО вычитаться из таймера ожидания ответа модели

3.6.2. Таймаут защищает от зависания **модели** (нет ответа от API), а НЕ от долгого выполнения инструментов (у инструментов собственные таймауты)
