...
4.5. В MVP системный промпт ДОЛЖЕН содержать краткое описание допустимой Markdown-разметки
     (заголовки, параграфы, жирный/курсив/зачеркнутый, ссылки/автоссылки, цитаты,
     списки включая task lists, код включая fenced-блоки с языком, таблицы,
     горизонтальные разделители, изображения, диаграммы Mermaid, математика через $$)
     и краткую подсказку о принудительном переносе строки (две пробела или завершающий `\`)
