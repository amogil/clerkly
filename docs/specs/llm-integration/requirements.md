...
8.9. КОГДА пользователь отправляет новое `kind:user` сообщение, ТО система ДОЛЖНА финализировать все non-terminal `tool_call` записи этого агента (`done=0`) в terminal состояние (`cancelled` preferred, `error` fallback) с `done=1` ДО запуска нового pipeline.
