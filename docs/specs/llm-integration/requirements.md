...
4.5. Системный промпт ДОЛЖЕН содержать краткое описание допустимой Markdown-разметки (GFM)
     ...
     горизонтальные разделители, изображения, inline код, fenced-блоки с языком
...
### 9. Встроенные изображения в ответах LLM

**ID:** llm-integration.9

**User Story:** Как пользователь, я хочу видеть изображения, встроенные в ответ LLM, чтобы получать визуальные элементы прямо в чате.

#### Критерии Приемки

9.1. КОГДА ассистент отвечает с изображениями, ТО пользователь ДОЛЖЕН видеть изображения встроенными в текст ответа.

9.2. ПОКА изображение загружается, пользователь ДОЛЖЕН видеть визуальный плейсхолдер.

9.3. КОГДА изображение успешно загружено, ТО плейсхолдер ДОЛЖЕН быть заменён на изображение в тексте.

9.4. ЕСЛИ изображение не удалось загрузить или оно недоступно, ТО плейсхолдер ДОЛЖЕН быть удалён без отображения ошибки пользователю.

9.5. КОГДА изображение содержит ссылку, пользователь ДОЛЖЕН иметь возможность открыть её кликом по изображению.

9.6. КОГДА указан размер изображения, плейсхолдер ДОЛЖЕН отображаться с этим размером до загрузки.

9.7. КОГДА изображение загружается асинхронно, оно ДОЛЖНО появиться после успешной загрузки.


#### Функциональные Тесты

- `tests/functional/llm-chat.spec.ts` — "LLM ответ с `[[image:1]]` и `data.images[]` → `<img>` появляется после загрузки"
...
11.5. URL-поля Structured Output (`images[].url`, `images[].link`) ДОЛЖНЫ содержать валидные URL с протоколом `http` или `https`.
