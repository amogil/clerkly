...
9.5.1. `tool_call` с `toolName = "final_answer"` ДОЛЖЕН содержать:
  - опциональное поле `text` (текст ответа модели, полученный при вызове инструмента `final_answer`),
  - `summary_points` (опциональный список пунктов завершения).

9.5.1.1. Системная инструкция и описание инструмента `final_answer` ДОЛЖНЫ явно фиксировать, что `text` и `summary_points` соблюдают лимиты `llm-integration.9.5.2-9.5.4`.

9.5.1.1.1. Системная инструкция ДОЛЖНА явно фиксировать, что обычный текстовый ответ модели (`kind: llm`, `data.text`) используется для диалога в процессе выполнения задачи (уточнения, запросы к пользователю, промежуточные сообщения), а не для фиксации завершения.

9.5.1.1.2. Системная инструкция ДОЛЖНА явно фиксировать, что `final_answer` вызывается только когда модель уверена, что работа завершена.

9.5.1.1.3. КОГДА вызывается `final_answer`, поле `text` ДОЛЖНО явно сообщать о завершении работы, а `summary_points` ДОЛЖЕН перечислять решённые задачи.

9.5.1.2. Значения по умолчанию для отсутствующих полей `final_answer` ДОЛЖНЫ быть:
  - `text`: отсутствует (без автоподстановки значения на runtime-слое `llm-integration`);
  - `summary_points`: `[]`.

9.5.2. КОГДА поле `text` присутствует в `final_answer`, ТО оно ДОЛЖНО быть непустой строкой длиной не более 300 символов.
...
9.5.5. ЕСЛИ `final_answer` нарушает ограничения по `text` (когда присутствует)/`summary_points`, ТО система ДОЛЖНА считать такой `final_answer` невалидным и запустить retry/repair по правилам `llm-integration.12.*`.

9.5.6. ОТСУТСТВИЕ `summary_points` (или пустой массив) ДОЛЖНО считаться успешным `completed`; правила отображения определяются спецификацией `agents` (см. `agents.7.4`).
