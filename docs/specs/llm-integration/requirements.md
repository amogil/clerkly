...
9.5.1.1. Системная инструкция и описание инструмента `final_answer` ДОЛЖНЫ явно фиксировать:
  ...
  - КОГДА модель завершает turn через `final_answer`, она МОЖЕТ публиковать обычный markdown/text ответ, НО этот текст НЕ ДОЛЖЕН дублировать содержимое `final_answer.summary_points`;
  - КОГДА текстовый ответ модели содержит список/чеклист, семантически эквивалентный `final_answer.summary_points`, модель НЕ ДОЛЖНА публиковать такой дублирующий список;
  - КОГДА текстовый ответ модели содержит самостоятельные пояснения (контекст, ограничения, следующие шаги, оговорки), не эквивалентные пунктам `final_answer.summary_points`, модель МОЖЕТ публиковать такой текст как обычный `kind: llm`;
...
9.5.3.3. КОГДА модель завершает turn через `final_answer`, ТО она НЕ ДОЛЖНА публиковать в `kind: llm` текст, который дублирует `final_answer.summary_points`; при этом недублирующий explanatory текст ДОЛЖЕН оставаться допустимым.
