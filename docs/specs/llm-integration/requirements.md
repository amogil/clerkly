...
16.8.3. Нормализация candidate title ДОЛЖНА удалять непарные двойные кавычки (ASCII `"`, типографские `\u201C`/`\u201D`). Парные двойные кавычки (с соответствующей открывающей/закрывающей) ДОЛЖНЫ сохраняться. Одинарные кавычки и апострофы НЕ ДОЛЖНЫ затрагиваться.
