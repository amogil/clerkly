...
describe('MainPipeline.run()', () => {
  ...
  /* Preconditions: provider rejects with LLMRequestAbortedError (timeout) 4 consecutive times, no meaningful chunks
     Action: run pipeline
     Assertions: chat called 4 times (1 initial + 3 retries), exactly 1 kind:error with type=timeout
     Requirements: llm-integration.12.2.3 */
  it('retries timeout errors up to 3 times before creating kind:error with type=timeout', async () => {
    const { pipeline, llmProvider, messageManager } = makeMocks();
    llmProvider.chat.mockRejectedValue(
      new LLMRequestAbortedError('Model response timeout', new Error('aborted'))
    );
    ...
    expect(llmProvider.chat).toHaveBeenCalledTimes(4);
    const errorCreates = (messageManager.create as jest.Mock).mock.calls.filter(
      (call: unknown[]) => call[1] === 'error'
    );
    expect(errorCreates).toHaveLength(1);
  ...
  /* Preconditions: provider rejects with timeout once, then succeeds on second attempt
     Action: run pipeline
     Assertions: chat called 2 times, no kind:error, kind:llm persisted
     Requirements: llm-integration.12.2.3 */
  it('recovers from timeout on second attempt without creating kind:error', async () => {
    const { pipeline, llmProvider, messageManager } = makeMocks();
    llmProvider.chat
      .mockRejectedValueOnce(
        new LLMRequestAbortedError('Model response timeout', new Error('aborted'))
      )
      .mockResolvedValueOnce({ text: 'Recovered after timeout' });
    ...
    expect(messageManager.create).toHaveBeenCalledWith(
      ...
      'llm',
      expect.objectContaining({
        data: expect.objectContaining({ text: 'Recovered after timeout' }),
      }),
      1,
      true
    );
  ...
  /* Preconditions: provider rejects with non-timeout error (network) twice, no meaningful chunks
     Action: run pipeline
     Assertions: chat called 2 times (1 initial + 1 generic silent retry), kind:error with type=network
     Requirements: llm-integration.12.2.4 */
  it('limits non-timeout silent retry to 1 attempt (not affected by timeout retry budget)', async () => {
    const { pipeline, llmProvider, messageManager } = makeMocks();
    llmProvider.chat
  ...
  /* Preconditions: provider rejects with timeout once, signal is aborted mid-retry
     Action: run pipeline with signal that gets aborted after first failure
     Assertions: chat called 1 time, no retry due to aborted signal, no kind:error (abort path)
     Requirements: llm-integration.12.2.3 */
  it('does not retry timeout when signal is aborted after first failure', async () => {
    const { pipeline, llmProvider, messageManager } = makeMocks();
    const controller = new AbortController();

    llmProvider.chat.mockImplementation(async () => {
      controller.abort();
      throw new LLMRequestAbortedError('Model response timeout', new Error('aborted'));
    });
  ...
  /* Preconditions: provider emits running code_exec tool_call, then provider throws (triggering retry)
     Action: run pipeline where first attempt creates tool_call(running) and then fails with retryable error
     Assertions: running tool_call is finalized (done=true, error output) before attempt messages are hidden
     Requirements: llm-integration.11.6.2 */
  it('finalizes running code_exec tool_call with error output before retry setHidden', async () => {
    const { pipeline, llmProvider, messageManager } = makeMocks();
    let attempt = 0;
    ...
    llmProvider.chat.mockImplementation(
      async (_msgs: ChatMessage[], _opts: ChatOptions, onChunk: (c: ChatChunk) => void) => {
        ...
        if (attempt === 1) {
          onChunk({
            type: 'tool_call',
            callId: 'call-retry-code-exec',
            toolName: 'code_exec',
            arguments: { task_summary: 'Run code', code: 'console.log(1)' },
          });
          // Provider fails without completing the tool — triggers retry
          return { text: '' };
        }
        // Second attempt succeeds
        onChunk({ type: 'text', delta: 'Recovered' });
    ...
    // Find the update call that finalized the running tool_call
    const toolCallUpdates = (messageManager.update as jest.Mock).mock.calls.filter(
      (call: unknown[]) => {
        const payload = call[2] as { data?: { output?: { status?: string }; callId?: string } };
        return payload?.data?.callId === 'call-retry-code-exec' && call[3] === true;
      }
    );
    expect(toolCallUpdates).toHaveLength(1);

    const finalizedPayload = toolCallUpdates[0][2] as {
      data: {
        output: {
          status: string;
    ...
    expect(finalizedPayload.data.output.status).toBe('error');
    expect(finalizedPayload.data.output.error?.code).toBe('internal_error');
    expect(finalizedPayload.data.output.error?.message).toBe('Retried due to model error.');
    expect(finalizedPayload.data.output.stdout).toBe('');
    expect(finalizedPayload.data.output.stderr).toBe('');

    // Verify finalization happened BEFORE setHidden
    const updateMock = messageManager.update as jest.Mock;
    const setHiddenMock = messageManager.setHidden as jest.Mock;

    const finalizeOrder =
      updateMock.mock.invocationCallOrder[updateMock.mock.calls.indexOf(toolCallUpdates[0])];
    const firstSetHiddenOrder = setHiddenMock.mock.invocationCallOrder[0];
    expect(finalizeOrder).toBeLessThan(firstSetHiddenOrder);
  });

  /* Preconditions: provider emits running generic tool_call, then fails triggering retry
     Action: run pipeline where first attempt creates generic tool_call(running) and then fails
     Assertions: generic tool_call is finalized with generic error output shape (not code_exec shape)
     Requirements: llm-integration.11.6.2 */
  it('finalizes running generic tool_call with generic error output before retry', async () => {
    const { pipeline, llmProvider, messageManager } = makeMocks();
    let attempt = 0;
    ...
    const toolCallUpdates = (messageManager.update as jest.Mock).mock.calls.filter(
      (call: unknown[]) => {
        const payload = call[2] as { data?: { output?: { status?: string }; callId?: string } };
        return payload?.data?.callId === 'call-retry-generic' && call[3] === true;
      }
    );
    expect(toolCallUpdates).toHaveLength(1);
    ...
    expect(finalizedPayload.data.output.status).toBe('error');
    expect(finalizedPayload.data.output.content).toBe('Retried due to model error.');
  });

  /* Preconditions: no running tool_calls in attempt (e.g., invalid final_answer retry)
     Action: handleAttemptFailure triggers retry for invalid final_answer
     Assertions: no tool_call update calls, only setHidden calls for llm messages
     Requirements: llm-integration.11.6.2 */
  it('skips tool_call finalization when attempt has no running tool_calls (invalid final_answer retry)', async () => {
    const { pipeline, messageManager, llmProvider } = makeMocks();
    ...
    llmProvider.chat
      .mockImplementationOnce(
        async (_msgs: ChatMessage[], _opts: ChatOptions, onChunk: (c: ChatChunk) => void) => {
          onChunk({
            type: 'tool_call',
            callId: 'final-invalid-no-running',
            toolName: 'final_answer',
            arguments: {},
          });
          return { text: '' };
    ...
    // Invalid final_answer retry does NOT create running tool_call (it's buffered, not flushed)
    // so finalizeRunningToolCallsForAttempt should be a no-op
    const toolCallFinalizations = (messageManager.update as jest.Mock).mock.calls.filter(
      (call: unknown[]) => {
        const payload = call[2] as { data?: { output?: { error?: unknown } } };
        return payload?.data?.output?.error !== undefined;
      }
    );
