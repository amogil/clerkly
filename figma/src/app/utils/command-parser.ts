...
export function parseCommand(input: string): ParsedCommand {
  ...
  // Create project
  if ((lower.includes('создай проект') || lower.includes('create project')) && !lower.includes('задач')) {
    const name = extractName(input, ['создай проект', 'create project']);
    return { action: 'create', entity: 'project', params: { name } };
  }

  // Create task
  if (lower.includes('создай задачу') || lower.includes('добавь задачу') || lower.includes('create task') || lower.includes('add task')) {
  ...
  // Add contact
  if (lower.includes('добавь контакт') || lower.includes('создай контакт') || lower.includes('add contact') || lower.includes('create contact')) {
    const name = extractName(input, ['добавь контакт', 'создай контакт', 'add contact', 'create contact']);
...
function extractProjectName(input: string): string | undefined {
  const projectMatch = input.match(/в проект[е]?\s+["«»"]?([^"«»"]+)["«»"]?/i) || 
