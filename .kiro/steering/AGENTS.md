...
- ✅ При падении тестов с ошибками загрузки нативных модулей

#### Автоматический rebuild:

Команда `npm test` автоматически выполняет `npm run rebuild:node` перед запуском всех тестов.
...
# Интеграционные тесты
npm run rebuild:node && npm run test:integration
...
## Правило Отключения Тестов

**КРИТИЧЕСКИ ВАЖНО**: Тесты НЕЛЬЗЯ отключать (skip) без явного подтверждения пользователя.

### Требования:

- **Запрет на самостоятельное отключение**: Агент НЕ ДОЛЖЕН отключать тесты (использовать `.skip()`) без явного разрешения пользователя
- **Обязательное согласование**: Перед отключением любого теста НЕОБХОДИМО:
  1. Объяснить пользователю, почему тест не может быть исправлен
  2. Предложить альтернативные решения (перенос в другой тип тестов, рефакторинг)
  3. Получить явное подтверждение от пользователя
- **Приоритет исправления**: Всегда пытаться исправить тест, а не отключить его
- **Альтернативы отключению**:
  - Перенести в интеграционные тесты (если требуется реальный Electron)
  - Перенести в функциональные тесты (если требуются реальные задержки или окна)
  - Упростить тест, уменьшив количество итераций
  - Улучшить изоляцию состояния между итерациями

### Исключения:

Тест может быть отключен БЕЗ согласования только если:
- Тест уже был отключен ранее (`.skip()` уже присутствует в коде)
- Пользователь явно попросил отключить конкретный тест

## Правило Создания Временных Отчетов и Документов
...
- ❌ Создание файлов с описанием изменений или статуса проекта
- ❌ Любые файлы, которые дублируют информацию из существующих спецификаций

### Причины Запрета:

- **Шум в репозитории**: Временные файлы засоряют рабочее пространство
- **Избыточность**: Информация уже содержится в спецификациях (requirements.md, design.md, tasks.md)
- **Отсутствие ценности**: Пользователь может сам запросить отчет, если нужно
- **Нарушение workflow**: Создание файлов без запроса отвлекает от основной работы
...
- ✅ Спрашивать пользователя: "Создать отчет о выполненной работе?" перед созданием файла

### Исключения:

Временные отчеты могут быть созданы ТОЛЬКО если:
- Пользователь явно попросил создать отчет или summary
- Пользователь указал конкретное имя файла для создания
- Это часть требований проекта (указано в requirements.md или design.md)
