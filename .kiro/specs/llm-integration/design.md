...
Миграция заполняет `kind` из `payload_json` и убирает его из JSON:

```sql
UPDATE messages SET kind = json_extract(payload_json, '$.kind');
...
const TIMEOUT_MS = 120_000; // 2 минуты
...
if (response.status === 401 || response.status === 403) {
  throw new LLMError('auth', 'Invalid API key. Please check your settings.');
}
