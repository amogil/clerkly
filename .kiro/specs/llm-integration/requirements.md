...
3.7. **Rate limit (HTTP 429) — баннер с обратным отсчётом:**

  3.7.1. ВМЕСТО обычного `kind: error` пузыря при 429 ДОЛЖЕН отображаться баннер с обратным отсчётом (countdown) до автоматического повтора запроса

  3.7.2. Баннер ДОЛЖЕН содержать:
    - Текст: `"Rate limit exceeded. Retrying in N seconds..."`
    - Кнопку "Cancel" для отмены повтора

  3.7.3. КОГДА отсчёт достигает нуля, ТО `MainPipeline` ДОЛЖЕН автоматически повторить последний запрос с тем же `userMessageId`
