...
- [x] 10.4 Имплементировать синхронный процесс получения профиля при авторизации
...
- [x] 12.3 Добавить loader для синхронной загрузки профиля при авторизации
  ...
  - **Requirements:** ui.6.4
  - **Property:** 16, 17, 18
  - **Примечание:** Реализовано в Фазе 1 - loader интегрирован с OAuth flow через AuthWindowManager

- [x] 12.4 Расширить LoginError компонент для обработки ошибки profile_fetch_failed
...
- [x] 14.8 Тест: fetchProfileSynchronously() успешно получает профиль при авторизации
...
- [x] 14.9 Тест: fetchProfileSynchronously() очищает токены при ошибке
...
- [x] 17.5 Тест: синхронная загрузка профиля при авторизации (успех)
  ...
  - Эмулировать deep link callback с authorization code через app.emit('open-url')
...
- [x] 17.6 Тест: синхронная загрузка профиля при авторизации (ошибка)
  - Создать реальный OAuthClientManager, UserProfileManager и TokenStorageManager с DataManager
  - Мокировать Google OAuth Token API для успешного обмена code на токены
...
- [x] 18.7 Функциональный тест: should show loader during synchronous profile fetch
  ...
  - Закрыть приложение
  - **Requirements:** ui.6.4
  - **Property:** 10, 11, 12, 16, 17
  - **Note:** Property 16 (показ loader) и Property 17 (показ Dashboard) покрыты этим функциональным тестом

- [x] 18.8 Функциональный тест: should show LoginError when profile fetch fails
  - Запустить приложение без авторизации
  - Начать авторизацию через Google OAuth
  ...
  - **Requirements:** google-oauth-auth.3.7, ui.6.4, ui.6.5
  - **Property:** 13, 18
  - **Note:** Property 18 (показ LoginError при ошибке загрузки профиля) покрыто этим функциональным тестом

- [x] 18.9 Функциональный тест: should show error and keep cached data when fetch fails
  - Запустить приложение с предварительно сохраненными данными профиля в локальной базе данных
...
- [x] 18.10 Функциональный тест: should not allow editing profile fields
...
- [x] 18.11 Функциональный тест: should update profile data when changed in Google
  - Запустить приложение с авторизацией
...
- [x] 18.12 Функциональный тест: should show login screen and clear UI on logout
  - Запустить приложение с авторизацией
...
- [x] 21.3 Интегрировать NavigationManager с OAuth events
  ...
  - **Requirements:** ui.8.3, ui.8.4, ui.8.9
  - **Property:** 26, 27, 32
...
- [x] 25.4 Функциональный тест: should redirect to login after logout
  ...
  - **Requirements:** ui.8.4
  - **Property:** 27, 32
  - **Note:** Property 32 (перенаправление на Login после logout) покрыто этим функциональным тестом
...
- [x] 37.3 Интегрировать handleAPIRequest() с другими API клиентами
  ...
  - **Requirements:** ui.9.3, ui.9.4
  - **Property:** 35
  - **Примечание:** Других API клиентов не существует. OAuth endpoints (token exchange, refresh, revoke) не используют handleAPIRequest, так как они сами управляют токенами.
...
- [x] 38.1 Проверить существующую реализацию refreshAccessToken()
  ...
  - **Property:** 28, 33
  - **Примечание:** Автоматическое обновление токенов реализовано в handleAPIRequest(). Проверяется срок действия токена перед каждым запросом, и если истек - автоматически обновляется.
...
- [x] 38.2 Добавить автоматический триггер refresh при истечении токена
  ...
  - **Requirements:** ui.9.1, ui.9.2
  - **Property:** 33
  - **Примечание:** Реализовано в handleAPIRequest(). Токен автоматически обновляется перед каждым API запросом, если истек срок действия.
...
- [x] 39.2 Добавить тесты для одновременных запросов с 401
  ...
  - **Requirements:** ui.9.4
  - **Property:** 30
  - **Примечание:** Тест реализован в tests/unit/auth/APIRequestHandler.test.ts - "should handle multiple simultaneous 401 errors without race conditions"
...
- [x] 42.1 Функциональный тест: should automatically refresh expired access token
  ...
  - **Requirements:** ui.9.1, ui.9.2
  - **Property:** 28, 33
  - **Note:** Property 33 (автоматическое обновление токена при истечении) покрыто этим функциональным тестом
...
- [x] 42.2 Функциональный тест: should clear session and show login on 401 error
  ...
  - **Requirements:** ui.9.3
  - **Property:** 29, 34
  - **Note:** Property 34 (очистка токенов при ошибке авторизации) покрыто этим функциональным тестом
...
- [x] 42.3 Функциональный тест: should handle 401 from any API endpoint consistently
  ...
  - **Requirements:** ui.9.3, ui.9.4
  - **Property:** 29, 30, 35
  - **Note:** Property 35 (централизованная обработка ошибок авторизации) покрыто этим функциональным тестом
...
- [x] 42.4 Функциональный тест: should log authorization errors with context
  ...
  - **Property:** 31, 36
  - **Note:** Property 36 (логирование ошибок авторизации с контекстом) покрыто этим функциональным тестом
...
## ВАЖНО: Новая Функциональность (Требует Реализации)

Следующие задачи помечены как `[ ]` (не выполнены), так как требуют реализации нового кода:

### Синхронная Загрузка Профиля (Property 16-18)
- [x] Задача 10.4: Реализовать `fetchProfileSynchronously()` в UserProfileManager
- [x] Задача 12.3: Добавить loader во время синхронной загрузки профиля
- [x] Задача 12.4: Расширить LoginError для обработки 'profile_fetch_failed'

### Централизованная Обработка API Запросов (Property 29-31, 34-36)
- [x] Задача 37.1: Создать `handleAPIRequest()` с проверкой HTTP 401
- [x] Задача 37.2: Интегрировать с UserProfileManager
- [x] Задача 37.3: Интегрировать с другими API клиентами

### Автоматическое Обновление Токенов (Property 28, 33)
- [x] Задача 38.1: Проверить/улучшить `refreshAccessToken()`
- [x] Задача 38.2: Добавить автоматический триггер refresh

### Обработка Race Conditions (Property 30, 35)
- [x] Задача 39.1: Добавить защиту от множественных очисток токенов
- [x] Задача 39.2: Добавить тесты для одновременных 401 ошибок
...
- [x] 57.2 Применить форматирование к Calendar компоненту
  - Заменить прямое форматирование дат на DateTimeFormatter.formatDate()
  - Заменить форматирование даты+времени на DateTimeFormatter.formatDateTime()
  - Использовать существующие паттерны отображения
  - **Requirements:** ui.11.2, ui.11.5
  - **Примечание:** Уже выполнено - calendar-view.tsx использует DateTimeFormatter.formatDate() (строка 305)
...
- [x] 57.4 Применить форматирование к истории изменений
  - Заменить прямое форматирование дат на DateTimeFormatter.formatDate()
  - Заменить форматирование даты+времени на DateTimeFormatter.formatDateTime()
...
- [x] 57.5 Обновить логирование для использования централизованного Logger класса
...
- [x] 73.1 Функциональный тест: should isolate data between different users
...
- [x] 73.2 Функциональный тест: should restore user data after re-login
  - Запустить приложение, авторизоваться как User A
...
- [x] 73.3 Функциональный тест: should persist data after logout
  - Запустить приложение, авторизоваться как User A
...
- [x] 73.4 Функциональный тест: should filter data by user email
  - Запустить приложение, авторизоваться как User A
...
- [x] 73.5 Функциональный тест: should handle "No user logged in" error gracefully
  - Запустить приложение без авторизации
...
- [x] 73.6 Функциональный тест: should retry operation after token refresh
  - Запустить приложение с авторизацией
...
- [x] 74.1 Добавить JSDoc комментарии к DataManager (изоляция)
  - Документировать обновленные методы saveData, loadData, deleteData
  - Добавить примеры использования с изоляцией
  - Указать ссылки на требования в комментариях
  - **Requirements:** ui.12

- [x] 74.2 Добавить JSDoc комментарии к UserProfileManager (email caching)
  - Документировать getCurrentEmail() и обновленные методы
  - Указать ссылки на требования в комментариях
  - **Requirements:** ui.12

- [x] 74.3 Обновить таблицу покрытия требований в design.md
...
- [x] 75.1 Запустить автоматическую валидацию
  - Выполнить `npm run validate`
  - Исправить все ошибки TypeScript
  - Исправить все ошибки ESLint
  - Исправить форматирование Prettier
...
- [x] 75.2 Проверить покрытие тестами
  - Убедиться, что покрытие >= 85%
  - Убедиться, что все требования ui.12.x покрыты тестами
  - Обновить таблицу покрытия в design.md
  - **Requirements:** ui.12

- [x] 75.3 Проверить комментарии с требованиями
  - Убедиться, что все функции имеют комментарии // Requirements:
  - Убедиться, что все тесты имеют структурированные комментарии
  - Проверить корректность ссылок на требования
  - **Requirements:** ui.12

- [x] 75.4 Проверить изоляцию данных во всех компонентах
...
- [x] 76.4 Запустить функциональные тесты (при явной просьбе пользователя)
  - Запустить все функциональные тесты из фаз 1-9
  - Убедиться, что все тесты проходят
  - Исправить любые найденные проблемы
  - **Requirements:** ui.1-ui.12
  - **Результат:** ✅ 88 passed, 2 skipped (date-time-formatting), 0 failed



## Сводка Покрытия Свойств Корректности

Все 36 свойств корректности из design.md покрыты тестами. Ниже приведена сводка покрытия:

### Свойства, Покрытые Property-Based Тестами

- **Property 5**: Размер окна основан на размере экрана (задача 5.1)
- **Property 6**: Изменения состояния окна сохраняются (задача 5.3)
- **Property 7**: Round-trip сохранения и загрузки состояния (задача 5.2)

### Свойства, Покрытые Функциональными Тестами

#### Фаза 1-2: Window Management
- **Property 1**: Окно открывается размером с workAreaSize, но не максимизировано (задача 7.1)
- **Property 2**: Окно можно изменять в размере и максимизировать (задача 7.1)
- **Property 3**: Окно имеет пустой заголовок (задача 7.1)
- **Property 4**: Окно использует нативные macOS элементы управления (задача 7.1)

#### Фаза 3: Account Profile
- **Property 8**: Показ экрана логина для неавторизованных пользователей (задача 25.1)
- **Property 9**: Открытие системного браузера для авторизации (задача 25.2)
- **Property 10**: Кнопка авторизации остается активной (задача 18.3)
- **Property 11**: Показ loader при получении authorization code (задача 18.7)
- **Property 12**: Синхронный обмен кода и загрузка профиля (задача 18.7)
- **Property 13**: Показ Dashboard после успешной авторизации и загрузки профиля (задача 18.7)
- **Property 14**: Показ LoginError при ошибке авторизации (задача 18.9)
- **Property 15**: Синхронная загрузка профиля при авторизации (задача 18.7)
- **Property 16**: Показ loader при синхронной загрузке профиля (задача 18.7) ✓
- **Property 17**: Показ Dashboard после успешной загрузки профиля (задача 18.7) ✓
- **Property 18**: Показ LoginError при ошибке загрузки профиля (задача 18.8) ✓
- **Property 19**: Сохранение данных профиля при успешной загрузке (задача 18.6)
- **Property 20**: Отображение обязательных полей профиля (задача 18.6)
- **Property 21**: Read-only поля профиля (задача 18.10)
- **Property 22**: Автоматическое обновление при refresh token (задача 18.11)
- **Property 23**: Автоматическое обновление при запуске приложения (задача 17.3)

#### Фаза 4: Navigation
- **Property 24**: Очистка токенов и показ экрана логина при logout (задача 18.12)
- **Property 25**: Блокировка доступа к защищенным экранам (задача 25.3)
- **Property 26**: Перенаправление на Dashboard после успешной авторизации (задача 25.2)
- **Property 27**: Перенаправление на Login после logout (задача 25.4)
- **Property 32**: Перенаправление на Login после logout (задача 25.4) ✓

#### Фаза 5: Error Notifications
- **Property 20**: Показ уведомления при ошибке фонового процесса (задача 34.1)
- **Property 21**: Содержимое уведомления об ошибке (задача 34.1)
- **Property 22**: Автоматическое исчезновение уведомления (задача 34.2)
- **Property 23**: Логирование ошибок в консоль (задача 34.4)

#### Фаза 6: Token Management
- **Property 28**: Автоматическое обновление токена при истечении (задача 42.1)
- **Property 29**: Очистка токенов при ошибке авторизации (задача 42.2)
- **Property 30**: Централизованная обработка ошибок авторизации (задача 42.3)
- **Property 31**: Логирование ошибок авторизации с контекстом (задача 42.4)
- **Property 33**: Автоматическое обновление токена при истечении (задача 42.1) ✓
- **Property 34**: Очистка токенов при ошибке авторизации (задача 42.2) ✓
- **Property 35**: Централизованная обработка ошибок авторизации (задача 42.3) ✓
- **Property 36**: Логирование ошибок авторизации с контекстом (задача 42.4) ✓

### Примечания

- ✓ - Свойство добавлено в существующий функциональный тест
- Все 8 отсутствующих свойств (16-18, 32-36) теперь покрыты функциональными тестами
- Property-based тесты не требуются для этих свойств, так как они проверяют UI поведение и интеграцию компонентов
- Функциональные тесты обеспечивают более подходящее покрытие для этих свойств
