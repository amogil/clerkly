...
2. **Обработка ошибок авторизации (HTTP 401)**: При получении ошибки HTTP 401 Unauthorized от любого API (Google UserInfo, Calendar, Tasks и т.д.), система должна:
   ...
   - Показать экран логина (LoginError компонент) с сообщением "Сессия истекла. Пожалуйста, войдите снова."
...
9.3. WHEN любой API запрос (Google UserInfo, Calendar, Tasks и т.д.) возвращает ошибку HTTP 401 Unauthorized, THE система SHALL:
   ...
   - Предоставить пользователю возможность повторной авторизации через кнопку "Try Again"
...
9.6. THE система SHALL НЕ показывать технические детали ошибки авторизации пользователю (только понятное сообщение "Сессия истекла")
