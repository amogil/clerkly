...
10.4. КОГДА пользователь изменяет API Key, ТО изменения ДОЛЖНЫ автоматически сохраняться в базу данных с debounce 500ms после последнего изменения

10.5. КОГДА пользователь изменяет LLM Provider (выпадающий список), ТО:
...
10.6. КОГДА пользователь очищает поле API Key (пустая строка), ТО запись для текущего провайдера ДОЛЖНА быть удалена из базы данных

10.7. Система ДОЛЖНА НЕ показывать визуальный индикатор сохранения (сохранение происходит молча в фоне)

10.8. КОГДА сохранение в базу данных не удается, ТО система ДОЛЖНА показать уведомление об ошибке через стандартный механизм обработки ошибок (см. ui.7)

10.9. Система ДОЛЖНА пытаться зашифровать API ключ перед сохранением используя `safeStorage.encryptString()` из Electron

10.10. КОГДА шифрование недоступно (например, на некоторых Linux системах), ТО система ДОЛЖНА сохранить API ключ без шифрования

10.11. Каждый LLM провайдер ДОЛЖЕН иметь отдельное хранилище для своего API ключа:
...
10.12. Система ДОЛЖНА сохранять метаданные о шифровании для каждого провайдера отдельно:
...
10.13. LLM Provider ДОЛЖЕН храниться в таблице `user_data` с ключом `ai_agent_llm_provider`

10.14. КОГДА пользователь переключается между провайдерами, ТО:
...
10.15. КОГДА приложение запускается, ТО настройки AI Agent ДОЛЖНЫ загружаться из базы данных текущего пользователя:
...
10.16. КОГДА настройки не найдены в базе (первый запуск или после очистки), ТО значения по умолчанию ДОЛЖНЫ быть:
   - LLM Provider: `openai`
   - API Key: пустая строка с placeholder "Enter your API key"

10.17. КОГДА API ключ загружается из базы для конкретного провайдера, ТО система ДОЛЖНА:
...
10.18. Система НЕ ДОЛЖНА выполнять валидацию формата API ключа

10.19. UI ДОЛЖЕН отображать текст: "Your API key is stored securely. It will only be used to communicate with your selected LLM provider."

10.20. Кнопка "Test Connection" ДОЛЖНА присутствовать в UI, но НЕ ДОЛЖНА быть функциональной (placeholder для будущей функциональности)

10.21. Система ДОЛЖНА использовать существующие IPC каналы для работы с настройками:
   - `save-data` для сохранения LLM провайдера и API ключей
   - `load-data` для загрузки настроек
   - `delete-data` для удаления API ключей

10.22. КОГДА пользователь выходит из системы (logout), ТО настройки AI Agent ДОЛЖНЫ сохраняться в базе данных, привязанные к email пользователя

10.23. КОГДА другой пользователь авторизуется (другой email), ТО система ДОЛЖНА загружать настройки этого пользователя из базы данных

10.24. КОГДА пользователь A выходит, пользователь B входит, затем A снова входит, ТО настройки пользователя A ДОЛЖНЫ восстановиться из базы данных (включая все сохраненные ключи для всех провайдеров)

**Примечание:** Изоляция данных между пользователями реализуется через добавление колонки `user_email` в таблицу `user_data` (см. ui.12).
