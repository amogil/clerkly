...
4.13.4. При отправке сообщения пользователем автоскролл ДОЛЖЕН срабатывать ВСЕГДА (независимо от позиции скролла)

4.13.5. Автоскролл ДОЛЖЕН использовать `scrollIntoView({ behavior: 'smooth' })` для плавной прокрутки
...
4.14.3. КОГДА пользователь возвращается к агенту с сохраненной позицией, ТО позиция скролла ДОЛЖНА восстанавливаться к сохраненному значению

4.14.4. КОГДА пользователь переключается на агента БЕЗ сохраненной позиции (первый визит), ТО ДОЛЖЕН выполняться автоскролл к последнему сообщению

4.14.5. Позиции скролла ДОЛЖНЫ храниться в Map<agentId, scrollTop> в памяти компонента (НЕ в localStorage)

4.14.6. При создании нового сообщения пользователем сохраненная позиция ДОЛЖНА сбрасываться (автоскролл к низу)
