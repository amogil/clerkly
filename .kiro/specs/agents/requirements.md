...
4.13.2. При отправке сообщения пользователем автоскролл ДОЛЖЕН срабатывать ВСЕГДА (независимо от позиции скролла)
...
4.14.1. КОГДА пользователь переключается на другого агента, ТО чат предыдущего агента ДОЛЖЕН скрываться (CSS `display: none` или `hidden`), но НЕ размонтироваться
...
13.2. Основной интерфейс ДОЛЖЕН показываться ТОЛЬКО после того, как все чаты всех агентов загружены (пока идёт загрузка — показывается лоадер)
...
13.8. При монтировании `AgentChat` ДОЛЖНЫ загружаться последние 50 сообщений агента через `messages:list-paginated`

13.9. КОГДА пользователь прокручивает чат к верхней границе, ТО ДОЛЖНЫ подгружаться следующие 50 сообщений (ленивая загрузка вверх)

13.10. Лоадер при старте приложения ДОЛЖЕН показываться пока хотя бы один `AgentChat` ещё загружает начальный чанк сообщений
