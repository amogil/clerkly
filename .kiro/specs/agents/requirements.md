...
1.4.4. Переход агента в начало списка ДОЛЖЕН сопровождаться плавной spring анимацией с параметрами: stiffness: 400, damping: 30, mass: 0.8
...
4.13.2. Автоскролл ДОЛЖЕН срабатывать ТОЛЬКО если пользователь находится в нижней трети чата

4.13.3. Для определения "находится в нижней трети" ДОЛЖНА использоваться проверка: `scrollHeight - scrollTop - clientHeight < clientHeight / 3`

4.13.4. При отправке сообщения пользователем автоскролл ДОЛЖЕН срабатывать ВСЕГДА (независимо от позиции скролла) с плавной анимацией

4.13.5. Автоскролл ДОЛЖЕН использовать `scrollIntoView({ behavior: 'smooth' })` для плавной прокрутки ТОЛЬКО при появлении новых сообщений (отправка пользователем, ответ агента)

4.13.6. Для автоскролла ДОЛЖЕН использоваться невидимый `div` с `ref={messagesEndRef}` в конце списка сообщений

4.13.7. Автоскролл ДОЛЖЕН прокручивать контейнер так, чтобы последнее сообщение было полностью видимо

4.13.8. Скроллбар ДОЛЖЕН быть визуально ненавязчивым и не отвлекать от содержимого

4.13.9. Скроллбар ДОЛЖЕН появляться при взаимодействии пользователя со скроллом (колесо мыши, тачпад, перетаскивание скроллбара)

4.13.10. Скроллбар ДОЛЖЕН автоматически скрываться после окончания скролла

4.13.11. Для реализации скроллбара и автоскролла ДОЛЖЕН использоваться компонент `Conversation` из AI Elements (обёртка над `use-stick-to-bottom`)

4.14. Позиция скролла ДОЛЖНА сохраняться для каждого агента независимо

4.14.1. КОГДА пользователь прокручивает чат агента, ТО позиция скролла (`scrollTop`) ДОЛЖНА сохраняться в памяти для этого агента

4.14.2. КОГДА пользователь переключается на другого агента, ТО позиция скролла предыдущего агента ДОЛЖНА сохраняться

4.14.3. КОГДА пользователь возвращается к агенту с сохраненной позицией, ТО позиция скролла ДОЛЖНА восстанавливаться к сохраненному значению МГНОВЕННО (без анимации, `behavior: 'auto'`)

4.14.4. КОГДА пользователь переключается на агента БЕЗ сохраненной позиции (первый визит), ТО ДОЛЖЕН выполняться автоскролл к последнему сообщению МГНОВЕННО (без анимации, `behavior: 'auto'`)

4.14.5. Позиции скролла ДОЛЖНЫ храниться в памяти компонента (НЕ в localStorage). Допустимо хранить ID последнего видимого сообщения вместо `scrollTop` если `Conversation` не предоставляет прямой доступ к `scrollTop`

4.14.6. При создании нового сообщения пользователем сохраненная позиция ДОЛЖНА сбрасываться (автоскролл к низу с плавной анимацией)

4.14.7. Восстановление позиции ДОЛЖНО происходить после загрузки сообщений агента, но ДО рендеринга UI

4.14.8. При первой загрузке приложения активный агент ДОЛЖЕН открываться с позицией скролла на последнем сообщении МГНОВЕННО (без визуального скролла, `behavior: 'auto'`)
...
## 15. AI Elements интеграция

### Описание

Область сообщений и поле ввода мигрируются на AI Elements компоненты и AI SDK v5.

### Критерии Приемки

15.1. Для управления скроллом ДОЛЖЕН использоваться компонент `Conversation` из AI Elements (обёртка над `use-stick-to-bottom`)

15.2. Автоскролл ДОЛЖЕН срабатывать при новых сообщениях если пользователь находится внизу чата

15.3. КОГДА пользователь прокрутил вверх, ТО автоскролл НЕ ДОЛЖЕН срабатывать при новых сообщениях

15.4. Кнопка "scroll to bottom" ДОЛЖНА появляться когда пользователь не находится внизу чата (`ConversationScrollButton`)

15.5. История сообщений ДОЛЖНА загружаться лениво: последние 50 сообщений при открытии агента, остальные — при скролле вверх

15.6. `useAgentChat` ДОЛЖЕН заменить `useMessages` как основной хук управления сообщениями, используя `useChat` из `@ai-sdk/react` с кастомным `IPCChatTransport`

15.7. `IPCChatTransport` ДОЛЖЕН конвертировать IPC-события (`MESSAGE_CREATED`, `MESSAGE_UPDATED`, `MESSAGE_LLM_REASONING_UPDATED`) в `ReadableStream<UIMessageChunk>` для `useChat`

15.8. Компонент `AgentMessage` ДОЛЖЕН заменить `MessageBubble`, сохраняя все `data-testid` атрибуты и визуальный стиль

15.9. Компонент `AgentPromptInput` ДОЛЖЕН заменить `ChatInput` + `AutoExpandingTextarea`, сохраняя все поведение и `data-testid="auto-expanding-textarea"`

15.10. `AgentHeader` ДОЛЖЕН оставаться без изменений

15.11. `RateLimitBanner` ДОЛЖЕН оставаться отдельным компонентом (не `AgentMessage`), рендерясь внутри `Conversation`
