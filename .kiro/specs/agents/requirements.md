...
4.13. ДОЛЖЕН выполняться автоскролл к последнему сообщению при создании сообщения пользователя

4.13.1. Автоскролл ДОЛЖЕН срабатывать ТОЛЬКО при создании сообщения с `kind: 'user'`

4.13.2. Автоскролл НЕ ДОЛЖЕН срабатывать при создании сообщений других типов (`llm`, `tool_call`, `final_answer`, и т.д.)

4.13.3. Автоскролл ДОЛЖЕН использовать `scrollIntoView({ behavior: 'smooth' })` для плавной прокрутки

4.13.4. Для автоскролла ДОЛЖЕН использоваться невидимый `div` с `ref={messagesEndRef}` в конце списка сообщений

4.13.5. Функция `scrollToBottom()` ДОЛЖНА вызываться явно после создания сообщения с `kind: 'user'`

4.13.6. Автоскролл ДОЛЖЕН прокручивать контейнер так, чтобы последнее сообщение было полностью видимо
