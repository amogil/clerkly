...
1. **UI отображает настройки из базы данных**: Компонент Settings читает настройки LLM Provider и другие настройки непосредственно из локальной базы данных (SQLite через DataManager). UI не хранит собственное состояние настроек, кроме временного состояния загрузки/ошибок.
...
**Примечание:** Изоляция данных LLM Provider между пользователями (сохранение и восстановление настроек при смене пользователя) реализуется через user-data-isolation.1 (добавление колонки `user_email` в таблицу `user_data`). Отдельные требования для изоляции настроек LLM Provider не нужны.
