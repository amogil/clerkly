...
2. Проверь, есть ли PR по этой задаче (`gh pr list --state all --search "<N>" --json number,title,state,labels`). Если PR существует:
   - Если на PR нет метки `new` — поставь её перед началом работы (`gh pr edit <PR> --add-label new`)
   - Замени метку `new` на `analysis` (`gh pr edit <PR> --add-label analysis --remove-label new`)
...
2. Запушить ветку
3. Если PR не существует — создать PR (draft) с меткой `analysis`. Если PR существует и не draft — перевести в draft (`gh pr ready <PR> --undo`). В описании PR кратко указать: ссылку на issue, суть задачи, ссылку на файл плана в репозитории
...
5. Определить финальную метку PR:
   - **`analysis review`** — план готов к реализации: нет открытых вопросов, нет незакрытых review threads
   - **`analysis`** — есть открытые вопросы или незакрытые threads, требуется ответ пользователя
6. Установить финальную метку на PR (`gh pr edit <PR> --add-label "<метка>" --remove-label new,analysis,"analysis review"`)
7. Вернуть ссылку на PR (и список открытых вопросов, если они есть)
