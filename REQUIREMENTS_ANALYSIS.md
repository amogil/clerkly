...
### 4.2. Технический Долг: Database Migration Strategy ⚠️

**Проблема:** Требование "пересоздать базу" может привести к потере данных

**Влияние:**
- Текущий подход: пересоздание базы вручную
- Пользователи потеряют все данные при обновлении (когда появятся)
...
**Рекомендация:**
1. Оставить текущий подход для MVP
2. Перед production release: добавить migration scripts
3. Определить стратегию миграции (ALTER TABLE, версионирование схемы)
...
1. ~~**Заменить "пересоздание базы" на миграцию**~~ ⚠️ ОТЛОЖЕНО ДО PRODUCTION
   - Текущий подход приемлем для MVP (пользователей нет)
   - Добавить migration scripts перед production release
...
7. ~~**Уточнить обработку race conditions**~~ ✅ ВЫПОЛНЕНО
   - error-notifications.1.5: Добавлены исключения для race conditions
   - Определено общее правило обработки

8. ~~**Уточнить запуск тестов**~~ ✅ ВЫПОЛНЕНО
   - clerkly.2.5-2.6: Явно разделены модульные/property-based и функциональные тесты
   - Добавлено примечание о причине разделения
...
- 5 конфликтов исправлены (архитектурный принцип, window state, OAuth tokens, race conditions, test execution)
- 1 критичная проблема требует решения (API key validation)
- 1 технический долг приемлем для MVP (database migration)
