...
### 4.1. КРИТИЧНО: API Key Validation UX ❌

**Проблема:** Отсутствует обработка невалидных API ключей
...
1. **Добавить требования о валидации API ключей** (settings)
   - Базовая валидация формата
   - UX для уведомления об ошибках
   - Функциональность "Test Connection"

2. **Завершить спецификацию visual-design**
   - Удалить неиспользуемые термины из глоссария
   - Или добавить требования позже, когда станут актуальны

3. **Опциональные улучшения тестирования** (не критично)
...
4. **Перейти на Google User ID для изоляции данных** (GitHub issue #16)
   - Использовать `sub` claim вместо email
   - Реализовать до production release
   - Приемлемо для MVP

5. **Добавить migration scripts для базы данных**
...
### Непротиворечивость: 96/100
- Найдено 6 потенциальных конфликтов
- 5 конфликтов исправлены
- 1 проблема требует решения (API key validation UX)
- Технический долг приемлем для MVP

### Общая Оценка: 96/100

**Вывод:** Требования хорошо структурированы и покрывают основную функциональность. Основная проблема - отсутствие валидации API ключей. Технический долг (email vs User ID, database migration) задокументирован и приемлем для MVP.
